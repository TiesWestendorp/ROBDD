class ROBDD {
  ...
  toJSON() {
    return (function _toJSON(bdd) {
      if (bdd.value !== undefined) return String(bdd.value)
      return { label: bdd._label, then: _toJSON(bdd._then), else: _toJSON(bdd._else) }
    })(this)
  }
   ...
   */
  numberOfSatisfyingAssignments(variableLabels) {
    if (this === ROBDD.True) return Math.pow(2, variableLabels.length)
    if (this === ROBDD.False) return 0

    const sortedVariableLabels = [...variableLabels]
    sortedVariableLabels.sort()
    const indicesByVariableLabel = new Map()
    sortedVariableLabels.forEach((label, index) => indicesByVariableLabel.set(label, index))

    // The root node need not be the first (sorted) variable label. All skipped decision variables
    // could be assigned any value.
    const skippedVariableLabelsFactor = Math.pow(2, indicesByVariableLabel.get(this._label))

    const count = new Map([[ROBDD.False, 0], [ROBDD.True, 1]])
    return skippedVariableLabelsFactor * (function _numberOfSatisfyingAssignments(bdd) {
      if (count.has(bdd)) return count.get(bdd)
      if (!variableLabels.includes(bdd._label)) throw "Unexpected variable label"

      const labelIndex = indicesByVariableLabel.get(bdd._label)
      let thenFactor, elseFactor

      if (bdd._then === ROBDD.True) {
        thenFactor = Math.pow(2, variableLabels.length - labelIndex - 1)
      } else if (bdd._then === ROBDD.False) {
        thenFactor = 0
      } else {
        thenFactor = Math.pow(2, indicesByVariableLabel.get(bdd._then._label) - labelIndex - 1)
      }

      if (bdd._else === ROBDD.True) {
        elseFactor = Math.pow(2, variableLabels.length - labelIndex - 1)
      } else if (bdd._else === ROBDD.False) {
        elseFactor = 0
      } else {
        elseFactor = Math.pow(2, indicesByVariableLabel.get(bdd._else._label) - labelIndex - 1)
      }

      const result =  thenFactor * _numberOfSatisfyingAssignments(bdd._then) + elseFactor * _numberOfSatisfyingAssignments(bdd._else)
      count.set(bdd, result)
      return result
    })(this, sortedVariableLabels)
  }

  /**
   * Generates all assignments (as Map of variable label to Boolean). Since the ROBDD's are agnostic
   * of all existing labels, the variableLabels must be specified. (E.g. the Boolean formula `True` has
   * 4 solutions in the solution space consisting of two decision variables.)
   */
  *solutions(variableLabels) {
    function* generateMappedBooleanPermutations(array) {
      const n = array.length
      for (let i = 0; i < (1 << n); i++) { // Iterate over all 2^n permutations
        const permutation = new Map()
        for (let j = 0; j < n; j++) { // Use i as a bitmask
          permutation.set(array[j], Boolean((i >> (n - j - 1)) & 1))
        }
        yield permutation
      }
    }
    yield *(function* _solutions(bdd, labeling) {
      if (bdd.isTerminal && bdd.value) {
        // The decisions variables that have not been assigned by the time we reach `True` can take any value
        const unassignedLabels = Array.from(new Set(variableLabels).difference(new Set(labeling.keys())))
        for(const permutation of generateMappedBooleanPermutations(unassignedLabels)) {
          yield new Map([...labeling, ...permutation])
        }
      }
      if (!bdd.isTerminal) {
        if (!variableLabels.includes(bdd._label)) throw "Unexpected variable label"

        yield *_solutions(bdd._then, new Map([...labeling, [bdd._label, true]]))
        yield *_solutions(bdd._else, new Map([...labeling, [bdd._label, false]]))
      }
    })(this, new Map())
  }
   ...
   *    maximize w_1 x_1 + ... + w_n x_n
   *    such that f(x_1, ..., x_n) = 1.
   */
  optimalSolution(weightsByVariableLabels) {
    const cache = new Map()
    let [value, labeling] = (function _optimalSolution(bdd) {
      if (cache.has(bdd)) {
        return cache.get(bdd)
      } else {
        if (bdd.isTerminal) {
          if (bdd.value) {
            // Handle ROBDD.True case
            return [0, Infinity, new Map()]
          } else {
            // Handle ROBDD.True case
            return [-Infinity, Infinity, null]
          }
        } else if (!bdd.isTerminal) {
          // Handle nonterminal case
          const [thenValue, nextThenLabel, thenLabeling] = _optimalSolution(bdd._then)
          const [elseValue, nextElseLabel, elseLabeling] = _optimalSolution(bdd._else)
          
          unassignedThenLabels = weightsByVariableLabels.keys().filter(label => bdd._label < label && label < nextThenLabel)

          const newValue = thenValue + weightsByVariableLabels.get(bdd._label)
          if (newValue >= elseValue) {
            const result = [newValue, bdd._label,  new Map([...thenLabeling, [bdd._label, true]])]
            cache.set(bdd, result)
            return result
          } else {
            const result = [elseValue, bdd._label, new Map([...elseLabeling, [bdd._label, false]])]
            cache.set(bdd, result)
            return result
          }
        }
    ...
    if (labeling == null) throw "No solution exists"

    // TODO: this needs to be done inside the loop: Unassigned labels should be set to true if their weight is positive, and false otherwise
    const unassignedLabels = Array.from(new Set(weightsByVariableLabels.keys()).difference(new Set(labeling.keys())))
    for(const unassignedLabel of unassignedLabels) {
      const weight = weightsByVariableLabels.get(unassignedLabel)
      if (weight > 0) {
        value += weight
        labeling.set(unassignedLabel, true)
      } else {
        labeling.set(unassignedLabel, false)
      }
    }
    return [value, labeling]
  }
  ...
  static andN(As) {
    ...
    return ROBDD.and(ROBDD.andN(As.slice(0, As.length/2)), ROBDD.andN(As.slice(As.length/2)))
  }
  static orN(As) {
    if (As.length === 0) return ROBDD.True
    if (As.length === 1) return As[0]
    ...
    return ROBDD.or(ROBDD.orN(As.slice(0, As.length/2)), ROBDD.orN(As.slice(As.length/2)))
  }
