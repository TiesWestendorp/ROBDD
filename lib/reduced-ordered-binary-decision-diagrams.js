class ROBDD {
  ...
  numberOfSatisfyingAssignments(variableLabels) {
    const copiedVariableLabels = [...variableLabels]
    copiedVariableLabels.sort()
    const indicesByVariableLabel = new Map()
    copiedVariableLabels.forEach((label, index) => indicesByVariableLabel.set(label, index))

    const count = new Map([[ROBDD.False, 0], [ROBDD.True, 1]])
    return (function _numberOfSatisfyingAssignments(bdd) {
