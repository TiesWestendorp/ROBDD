    ...
    let [value, labeling] = (function _optimalSolution(bdd) {
      if (cache.has(bdd)) {
        ...
        if (bdd.isTerminal) {
          ...
          const [thenValue, thenLabeling] = _optimalSolution(bdd._then)
          const [elseValue, elseLabeling] = _optimalSolution(bdd._else)
          
          const newValue = thenValue + weightsByVariableLabels.get(bdd._label)
          if (newValue >= elseValue) {
            const result = [newValue,  new Map([...thenLabeling, [bdd._label, true]])]
            cache.set(bdd, result)
            console.warn(cache)
            return result
          } else {
            const result = [elseValue, new Map([...elseLabeling, [bdd._label, false]])]
            cache.set(bdd, result)
            console.warn(cache)
            return result
          }
