...
//    const count = new Map([[ROBDD.False, 0], [ROBDD.True, 1]])
    return (function _numberOfSatisfyingAssignments(bdd, number_of_variables) {
      if (bdd === ROBDD.True)  return Math.pow(2, number_of_variables)
      if (bdd === ROBDD.False) return 0
      if (!variableLabels.includes(bdd._label)) throw "Unexpected variable label"
      return _numberOfSatisfyingAssignments(bdd._then, number_of_variables - 1) + _numberOfSatisfyingAssignments(bdd._else, number_of_variables - 1)
    })(this, variableLabels.length)
  }
